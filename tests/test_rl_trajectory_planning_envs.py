...
from magbotsim.rl_envs.trajectory_planning.long_horizon_global_trajectory_planning_env import LongHorizonGlobalTrajectoryPlanningEnv
...
def test_jerk_actuator(num_movers, mover_mass, jerk, num_cycles, test_x, test_y):
    ...
    for step in range(0, num_steps):
        ...
        for idx_mover in range(0, num_movers):
            pos_mj_actuator[step, idx_mover : idx_mover + 2] = env.get_mover_qpos(mover_names=env.mover_names[idx_mover], add_noise=False)[
                0, :2
            ]
            velo_mj_actuator[step, idx_mover : idx_mover + 2] = env.get_mover_qvel(mover_names=env.mover_names[idx_mover], add_noise=False)[
                0, :2
            ]
            acc_mj_actuator[step, idx_mover : idx_mover + 2] = env.get_mover_qacc(mover_names=env.mover_names[idx_mover], add_noise=False)[
                0, :2
            ]
